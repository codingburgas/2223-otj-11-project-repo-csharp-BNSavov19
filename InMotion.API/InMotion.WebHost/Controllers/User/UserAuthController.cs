...
using InMotion.WebHost.Models.Auth;
...
public class UserAuthController : ControllerBase
{
    ...
    public async Task<IActionResult> RegisterAsync([FromForm] RegisterModel model)
    {
        ...
        if (model.AvatarImage is null)
        {
            avatarImageUrl = $"https://avatars.dicebear.com/api/initials/{model.FirstName![0]}{model.LastName![0]}{Guid.NewGuid()}.svg".Replace("-", "");
        }
        else
        {
            avatarImageUrl = await fileService.SaveImageAsync(model.AvatarImage, "useravatars");
        }
        
        var response = await this.authService.CreateUserAsync(model, avatarImageUrl);
    ...
    /// <summary>
    /// Sends email for reseting a password.
    /// </summary>
    ...
    public async Task<IActionResult> ForgotPasswordAsync([FromForm] ForgotPasswordModel model)
    {
        ...
        this.logger.LogInformation("User with email {Email} has succesfully send a password recovery email.", model.Email);
    ...
    /// </summary>
    /// <param name="model">Model for reseting a password.</param>
    /// <returns>Response with the result.</returns>
    [HttpPost]
